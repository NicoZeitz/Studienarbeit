...
unsafe extern "C" fn write_modeldata(val: *mut libc::c_void, buf: *mut libc::c_char) -> libc::c_int {
    ...
    let val = transmute::<_, &mut &mut dyn Write>(val);
...
impl Problem {
    ...
    pub fn new(rows: libc::c_int, cols: libc::c_int) -> Option<Self> {
        let ptr = unsafe { lp::make_lp(rows, cols) };
        cptr!(ptr)
    }

    /// Reads an lp-format model from `path`.
    pub fn read_lp<P: Deref<Target=CStr>, C: Deref<Target=CStr>>(path: &P, verbosity: Verbosity, initial_name: &C) -> Option<Self> {
        let ptr = unsafe { lp::read_LP(path.as_ptr().cast_mut(), verbosity as libc::c_int, initial_name.as_ptr().cast_mut()) };
        cptr!(ptr)
    }

    /// Read an mps-format model from `path` using the "free" formatting.
    pub fn read_freemps<P: Deref<Target=CStr>>(path: &P, options: MPSOptions) -> Option<Self> {
        let ptr = unsafe { lp::read_freeMPS(path.as_ptr().cast_mut(), options.bits()) };
        cptr!(ptr)
    }

    /// Read an mps-format model from `path` using the fixed formatting.
    pub fn read_fixedmps<P: Deref<Target=CStr>>(path: &P, options: MPSOptions) -> Option<Self> {
        let ptr = unsafe { lp::read_MPS(path.as_ptr().cast_mut(), options.bits()) };
        cptr!(ptr)
    }
    ...
    pub fn write_lp(&self, out: &mut dyn Write) -> bool {
    ...
    ///
    /// If there are any errors writing to `out`, `false` will be returned. Otherwise, `true`.
    pub fn write_fixedmps(&self, out: &mut dyn Write) -> bool {
    ...
    ///
    /// If there are any errors writing to `out`, `false` will be returned. Otherwise, `true`.
    pub fn write_freemps(&self, out: &mut dyn Write) -> bool {
    ...
    pub fn write_mps(&self, out: &mut dyn Write, formatting: libc::c_int) -> bool {
    ...
    pub fn add_column(&mut self, values: &[f64]) -> bool {
        ...
        1 == unsafe { lp::add_column(self.lprec, values.as_ptr().cast_mut()) }
    }
    ...
    pub fn add_column_scatter(&mut self, values: &[f64], indices: &[libc::c_int]) -> bool {
        ...
        1 == unsafe { lp::add_columnex(self.lprec, len as libc::c_int, values.as_ptr().cast_mut(), indices.as_ptr().cast_mut()) }
    }

    /// Read a column from the model.
    pub fn get_column(&self, values: &mut [f64], column: libc::c_int) -> bool {
        assert!(values.len() > self.num_rows() as usize);
    ...
    pub fn get_row(&self, values: &mut [f64], row: libc::c_int) -> bool {
        assert!(values.len() > self.num_cols() as usize);
    ...
    pub fn add_constraint(&mut self, coeffs: &[f64], target: f64, kind: ConstraintType) -> bool {
        assert!(coeffs.len() > self.num_cols() as usize);
        1 == unsafe { lp::add_constraint(self.lprec, coeffs.as_ptr().cast_mut(), kind as libc::c_int, target) }
    }
    ...
    pub fn add_sos_constraint(&mut self, name: &CStr, sostype: SOSType, priority: libc::c_int,
        ...
        0 != unsafe { lp::add_SOS(self.lprec, name.as_ptr().cast_mut(), sostype as libc::c_int, priority,
                             len as libc::c_int, variables.as_ptr().cast_mut(), weights.as_ptr().cast_mut()) }
    }
    ...
    pub fn set_integer(&mut self, col: libc::c_int, must_be_integer: bool) -> bool {
        1 == unsafe { lp::set_int(self.lprec, col, u8::from(must_be_integer)) }
    }
    ...
    pub fn set_objective_function(&mut self, coeffs: &[f64]) -> bool {
        assert!(coeffs.len() > self.num_cols() as usize);
        1 == unsafe { lp::set_obj_fn(self.lprec, coeffs.as_ptr().cast_mut()) }
    }
    ...
    pub fn scatter_objective_function(&mut self, coeffs: &[f64], indices: &[libc::c_int]) -> bool {
        ...
        1 == unsafe { lp::set_obj_fnex(self.lprec, len as libc::c_int, coeffs.as_ptr().cast_mut(), indices.as_ptr().cast_mut()) }
    }
    ...
    pub fn get_constraint_range(&self, row: libc::c_int) -> Option<f64> {
        if row > self.num_rows() + 1 {
            ...
            if (delta - self.get_infinite()).abs() < f64::EPSILON {
                None
            } else {
                Some(delta)
            }
    ...
    pub fn solve(&mut self) -> SolveStatus {
        use SolveStatus::{Degenerate, FeasibleFound, Infeasible, NoFeasibleFound, NotRun, NumericalFailure, Optimal, OutOfMemory, Presolved, ProcBreak, ProcFail, Suboptimal, Timeout, Unbounded, UserAbort};
    ...
    pub const unsafe fn from_lprec(lprec: *mut lp::lprec) -> Self {
        Self {
            lprec
        }
    }
    ...
    pub const fn to_lprec(&self) -> *mut lp::lprec {
        self.lprec
    }
    ...
    /// Sets the timeout.
    ///
    /// Always uses seconds as unit.
    pub fn set_timeout(&mut self, timeout: std::time::Duration) {
        let timeout = timeout.as_secs() as i32;
        unsafe { lp::set_timeout(self.lprec, timeout) }
    }

    /// Gets the timeout.
    pub fn get_timeout(&mut self) -> std::time::Duration {
        let timeout = unsafe { lp::get_timeout(self.lprec) };
...
impl Clone for Problem {
    fn clone(&self) -> Self {
        ...
        assert!(!ptr.is_null(), "OOM when trying to copy_lp");
        Self { lprec: ptr }
    }
}
...
mod tests {
    ...
    fn smoke() {
        ...
        assert_eq!(lp.solve(), super::SolveStatus::NotRun);
    }
}