...
impl ResBlock {
    pub fn new(num_hidden: usize, vb: VarBuilder) -> Result<Self> {
        let convolution_config = Conv2dConfig {
            padding: 1,
            ..Conv2dConfig::default()
        };

        let convolution_1 = conv2d(num_hidden, num_hidden, 3, convolution_config, vb)?;
        let batch_norm_1 = batch_norm(num_hidden, BatchNormConfig::default(), vb)?;
        let convolution_2 = conv2d(num_hidden, num_hidden, 3, convolution_config, vb)?;
        let batch_norm_2 = batch_norm(num_hidden, BatchNormConfig::default(), vb)?;
        Ok(Self {
            convolution_1,
            batch_norm_1,
            convolution_2,
