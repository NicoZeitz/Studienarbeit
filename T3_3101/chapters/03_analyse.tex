...
\begin{table}[!ht]
    \centering
    \begin{tabular}[t]{c|c|c|c}
        \raisebox{0pt}[0.35ex][0.35ex]{Flicken} & \adjustbox{valign=m}{\includegraphics[width=0.155\textwidth]{res/pictures/assets/00-front.png}} & \adjustbox{valign=m}{\includegraphics[width=0.155\textwidth]{res/pictures/assets/09-front.png}} & \adjustbox{valign=m}{\includegraphics[width=0.155\textwidth]{res/pictures/assets/21-front.png}} \\ \hline
        \makecell{Platzierungs-                                                                                                                                                                                                                                                                                                                       \\möglichkeiten} & $9 \cdot 8 \cdot 2 = 144$                                                                      & $8 \cdot 8 \cdot 1 = 64$                                                                       & $8 \cdot 8 \cdot 4 = 256$                                                                      \\ \hline
    \end{tabular}
    \vspace{5pt}
...
% balance >= 0
% zeitkosten <= 54
% t_i = vorherige t_is + time cost

% \\  Variablen \\ \\

% x = \left(x_0, \dots, x_{32}\right)^T \\
% t = \left(t_0, \dots, t_{32}\right)^T \text{Auswahlzeitpunkt auf Zeitplan} \\
% a = \left(a_0, \dots, a_{32}\right)^T \text{Flaechenvektor} \\
% i = \text{Button Income Vektor} \\
% z = \text{Zeitkostenvektor} \\
% c = \text{Kostenvektor}

% \\ Bedingung \\ \\

% max \overset{!}{=} \sum_{n=1}^{9} i^Tbi^{(n)} + 2 \cdot \left(\lambda\cdot 81 + (1 - \lambda) \cdot x^Ta\right) - x^Tc

% \\ \\ Nebenbedingungen \\ \\

% 0 \le \lambda      \le 1  \quad \text{ bzw. } \lambda \in \{0,1\}\\
% 0 \le x_i          \le 1  \quad \text{ bzw. } x_i \in \{0,1\}\\
% 0 \le t_i          \le 53 \quad \text{ bzw. } t_i \in \mathbb{N}_0 \cap [0,53] \\
% 0 \le bi^{(n)}_{i} \le 1  \quad \text{ bzw. } bi^{(n)}_{i} \in \{0,1\} \qquad \forall n \in \mathbb{N} \cap [1,9] \\

% \\

% x_i \le t_i \le 1000 \cdot x_i \\

% \\ Flaeche \\

% x^Ta \le 81 \\
% x^Ta - 76 + 1 \le 1000 \cdot \lambda \text{ bzw. } x^Ta - 76 < 1000 \cdot \lambda \\
...
% \\ Income \\
% c_n - t_i < 1000 \cdot bi^{(n)}_{i} \quad \forall n \in \mathbb{N} \cap [1,9] \text{ und } c=(5,11,17,23,29,35,41,47,53) \\

% \\ Balance \ge 0 \\

% ???\ c^T ?? \ge 5 + \quad \forall n \in \mathbb{N}_0 \cap [0,54] \\
...
% für z = 0 bis 54
%     alle c_i mit t_i <= z
%     summe c_i >= 5 + summe income_i mit passenden konstanten

%     alle kosten von t_i


% i = (3,2,4,5,1) Indexe nach Flickenauswahl
% alle unterschiedlich i_i != i_j

% sum t_i    = 1 + 4 + 5 + 10 + 14 + 35 = 69
% sum z_i    =     3 + 1 +  5 +  4 + 21 = 34
% cumsum z_i =     3 + 4 +  9 + 13 + 34 = 63

% sum t_i = cumsum z_i + 1
